...
\begin{abstract}
From a corpus of circa 50 million German news articles, this study curates a dataset of 31,368 articles on femicide. The curation process involves semantic search, query selection, and cosine-distance threshold selection based on stratified sampling, precision estimation, and bootstrap confidence intervals. 
%The curation process involves semantic search and manual validation of stratified samples for query and cosine-distance threshold selection. 
These methods are reproducible for similar research and downstream analysis. Using the final dataset, we present a spatial and temporal analysis of femicide-related media coverage to identify cases of over- or underreporting.
\end{abstract}
...
In the curation process of our dataset, we manually label 1,000 articles [\ref{sec:annotation}], select a high-performing semantic search query [\ref{sec:queryselection}], and determine a cosine distance threshold to
ensure a sufficiently high dataset precision [\ref{sec:threshold}]. The resulting dataset is then used to illustrate geographical and temporal patterns in German media coverage of femicide [\ref{sec:results}]. Further implications are also discussed [\ref{sec:conclusion}]. %does implcaitions mean what others can do that we didn't, otherwise mention that too. and can make this last parpgah a bit longer since we have the space ++ can this be more specific? Further implications of what?
\section{Data and Methods}\label{sec:methods}

\subsection{Data Source: German News Corpus}
\label{sec:datasource}
To build our dataset of news articles on femicide, we use a geolocated German news dataset developed by Kriesch and Losacker (\citeyear{kreischlosacker2025geolocated}). The corpus contains about 50 million German news articles extracted from CommonCrawl, with crawl dates between August 2016 and December 2023. The dataset consists of a SQLite database containing article information (including unique identifiers, URLs, publication and crawl dates, and geolocated data) and a Usearch vector database that stores a semantic embedding for each article.
 
To retrieve articles relevant to our topic, a natural-language query is encoded into a vector representation and compared to the stored article embeddings (32-bit floating-point precision), using cosine distance as a similarity measure. The search returns the top-$k$ articles whose embeddings are most similar to the query vector, where smaller cosine-distance values indicate higher semantic similarity. The identifiers of the retrieved articles are then used to retrieve metadata and geographic information from the SQLite database.

\subsection{Curation Pipeline}
\label{sec:pipeline}
Using the semantic retrieval and structured querying described above, we construct an initial dataset of 500{,}000 articles. We then reduce to approximately 365{,}000 articles in two filtering steps:
\begin{itemize}[label=--, leftmargin=1.4em, topsep=0pt, partopsep=0pt, itemsep=1pt, parsep=0pt]
  \item Articles must contain at least one German NUTS location (fr.\ \emph{Nomenclature des unités territoriales statistiques}). Each article is associated with the locations mentioned in its content. As articles published by German outlets may report on femicide cases outside Germany, this filter restricts the dataset to articles that reference at least one location within Germany.
...
\end{itemize}
After the two filtering steps, we apply a cosine-distance threshold of $t = 0.225$ and retain only articles with $\mathrm{cos\_dist} \le t$. This results in a dataset of 31{,}368 articles. Below, we explain how the query and the threshold were chosen, with both steps relying on sampling and manual annotation.
...
In addition to computing $\hat p(t)$ for a set of candidate thresholds, we construct confidence intervals to quantify uncertainty in $\hat p(t)$ by using a stratified bootstrap. For each bootstrap replicate $b=1,\dots,5000$, we draw $n_k$ samples with replacement from each $B_k$, and we recompute $\hat p(t)$ for all thresholds $t$ under consideration. This yields, for each threshold $t$, a bootstrap distribution
\[
...
0.210 & 7{,}815   & 0.978 & [0.930,\,1.000] \\
0.215 & 13{,}351  & 0.967 & [0.925,\,0.999] \\
0.220 & 21{,}192  & 0.931 & [0.889,\,0.966] \\
0.225 & 31{,}368  & 0.911 & [0.853,\,0.959] \\
0.230 & 44{,}232  & 0.899 & [0.846,\,0.944] \\
0.235 & 59{,}803  & 0.839 & [0.781,\,0.892] \\
0.240 & 78{,}943  & 0.801 & [0.745,\,0.854] \\
0.245 & 102{,}474 & 0.763 & [0.705,\,0.820] \\
0.250 & 132{,}083 & 0.677 & [0.615,\,0.737] \\
\bottomrule
\end{tabular}
\end{table}
...
Rachel Amruthaluri ran background research on original dataset structure, curation methods, validation methods, and testing methods.
...
Madeline Miller collected and prepared the query datasets, set up the manual annotation collection process, performed query selection and visualization, and set up the GitHub repository.
